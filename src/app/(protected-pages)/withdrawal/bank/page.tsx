...
// Symbol to ID mapping for conversion API
...
const BankTransferForm = () => {
  ...
  const validateCurrentTab = () => {
    ...
    if (activeTab === "currency") {
      if (!formData.amount) {
        ...
        const balance = balanceType === "Available"
  ...
  const fetchBundleDetails = async () => {
    try {
      const res = await axios.get(`${process.env.NEXT_PUBLIC_BACKEND_URL}/user/fees/bundle/fetch?category=Bank`, {
        headers: {
          Authorization: `Bearer ${token}`,
        }
      });
  ...
  useEffect(() => {
    const updateConversion = async () => {
      if (formData.amount && selectedWithdrawCurrency?.shortName) {
        const rate = await fetchConversionRate(selectedWithdrawCurrency.shortName, 'USD');
        if (rate) {
          setConversionData({
            cryptoAmount: formData.amount,
            usdAmount: (parseFloat(formData.amount) * rate).toFixed(2),
            rate
          });
        }
      }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      submitData.append('total', conversionData?.usdAmount)
      ...
      const response = await axios.post(`${process.env.NEXT_PUBLIC_BACKEND_URL}/user/withdraw/bank/create`, submitData, {
        headers: {
  ...
  const fetchCrypto = async () => {
    try {
      const cryptoRes = await axios.get(`${process.env.NEXT_PUBLIC_BACKEND_URL}/user/currency/fetch?type=Fiat`, {
      ...
      if (cryptoRes.data.data && cryptoRes.data.data.length > 0) {
        ...
        updateFormData("currencyId", cryptoRes.data.data[0].id?.toString() || '');
      }
  ...
  const handleWithdrawCurrencySelect = (key: string) => {
    ...
    if (selected) {
      ...
      updateFormData("currencyId", selected.currencyId?.toString() || '');
  ...
  const fetchWithdrawalHistory = async () => {
    ...
    try {
      const res = await axios.get(`${process.env.NEXT_PUBLIC_BACKEND_URL}/user/withdraw/fetch?withdrawType=Bank`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      })
      ...
      <div className="min-h-screen p-6  shadow-sm bg-white dark:bg-gray-800 rounded-lg">
        <Tabs defaultValue="Available" onChange={handleBalanceTypeChange}>
          <TabList>
            <TabNav
              value="Available"
              className={balanceType === "Available" ? "active" : ""}
            >
              Available
            </TabNav>
            <TabNav
              value="Locked"
              className={balanceType === "Locked" ? "active" : ""}
            >
              Locked
            </TabNav>
          </TabList>
          ...
          {['Available', 'Locked'].map((tab) => (
            <TabContent key={tab} value={tab} className={balanceType === tab ? "block" : "hidden"}>
                ...
                {activeTab === "currency" && (
                  <div className="space-y-4">
                    <div className="flex justify-between items-center">
                      <span className="">{balanceType}</span>
                      {/* <span className="">
                    ...
                    <div>
                      ...
                      <Dropdown
                        title={selectedWithdrawCurrency ? `${selectedWithdrawCurrency.shortName} - ${selectedWithdrawCurrency.fullName}` : 'Select Crypto Currency'}
                        trigger="click"
                      ...
                      {errors.currencyId && <p className="text-red-500 text-sm mt-1">{errors.currencyId}</p>}
                    </div>

                    <div>
                      <label className="block text-sm font-medium mb-1">
                        Amount ({selectedWithdrawCurrency?.shortName || 'Crypto'}) <span className="text-red-500">*</span>
                      </label>
                      <Input
                        type="number"
                        ...
                        onChange={(e: any) => updateFormData("amount", e.target.value)}
                        className="border border-gray-300 focus:ring-0  dark:bg-gray-800 rounded-lg"
                      />
                      {errors.amount && <p className="text-red-500 text-sm mt-1">{errors.amount}</p>}
                      {conversionLoading && (
                        <div className="text-sm text-blue-400 mt-1">Converting...</div>
                      )}
                      {conversionData && !conversionLoading && (
                        <div className="text-sm text-green-400 mt-1">
                          ≈ ${conversionData.usdAmount} USD
                        </div>
                      )}
                    </div>
                    ...
                    {shouldShowBundleForWithdraw && (
                      <div className='w-full mb-4'>
                        <label className='text-sm mb-2 block'>Fee Options: <span className="text-red-500">*</span></label>
                    ...
                    <div className="space-y-2 pt-4 border-t border-gray-300">
                      <div className="flex justify-between">
                        <span>Withdrawal Amount ({selectedWithdrawCurrency?.shortName || 'Crypto'}):</span>
                        <span>{formData.amount || "0"}</span>
                      </div>
                      <div className="flex justify-between">
                        <span>USD Equivalent:</span>
                        <span>${conversionData?.usdAmount || "0"}</span>
                      </div>
                      <div className="flex justify-between">
                    ...
                    <Button
                      ...
                      iconAlignment="end"
                      disabled={!formData.amount || (shouldShowBundleForWithdraw && !selectedFeeBundle)}
                    >
                      NEXT
                    </Button>
                  </div>
                  ...
                  <div className="space-y-3">
                    ...
                    <div>
                      ...
                      <Input
                        ...
                        className="w-full text-gray-300"
                      />
                      {errors.paymentReferenceNumber && <p className="text-red-500 text-sm mt-1">{errors.paymentReferenceNumber}</p>}
                    </div>
