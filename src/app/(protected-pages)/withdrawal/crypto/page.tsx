...
const Page = () => {
    ...
    const cryptoBundles = useMemo(() => {
        return (bundleDetails || [])
            .filter(
                (bundle) => (bundle.category || '').toLowerCase() === 'crypto',
            )
    ...
    const fetchConversionRate = useCallback(
        async (
            ...
            if (!fromCoinId || !toCoinId) {
                console.warn(
                    `No mapping found for symbols: ${fromSymbol} or ${toSymbol}`,
                )
                return null
            }
    ...
    const fetchCrypto = async () => {
        try {
            const cryptoRes = await axios.get(
                `${process.env.NEXT_PUBLIC_BACKEND_URL}/user/currency/user/fetch?type=Crypto`,
                {
    ...
    const calculateWithdrawFees = (amount: string): number => {
        ...
        } else if (userDetails?.withdrawFees) {
    ...
    const handleWithdrawClick = async (e: React.MouseEvent) => {
        ...
        if (shouldShowBundleForWithdraw && selectedFeeBundle) {
            const isEligible = isBundleEnabled(
                selectedFeeBundle,
                withdrawAmount,
            )
            if (!isEligible) {
                const message =
                    'Selected fee option is not eligible for this amount. Please choose a different fee option.'
                setErrorMessage(message)
                toast.error(message)
                return
            }
        ...
        try {
            ...
            formData.append(
                'currencyId',
                selectedWithdrawCurrency.currencyId?.toString() ||
                    selectedWithdrawCurrency.id ||
                    '',
            )
            ...
            if (
                selectedWithdrawCurrency.shortName === 'BTC' &&
                conversionData.usdAmount
            ) {
                formData.append('total', conversionData.usdAmount)
            } else {
                formData.append(
                    'total',
                    calculateTotalWithdrawAmount(withdrawAmount).toString(),
                )
            }
        ...
        },
        {
            ...
            cell: ({ row }) => (
                <span className="text-nowrap">
                    {Number(row.original.amount).toFixed(6)}{' '}
                    {row.original.currency?.shortName}
                </span>
            ),
        },
        {
            ...
            cell: ({ row }) => {
                const type =
                    ...
                    ((row.original.withdrawBank?.length ?? 0) > 0
                        ? 'Bank'
                        : (row.original.withdrawCrypto?.length ?? 0) > 0
                          ? 'Crypto'
                          : 'Unknown')
                return (
                    <span
                ...
                <div className="flex items-center justify-center">
                    <div className="w-full md:w-[60%]  p-6 shadow-sm bg-white dark:bg-gray-800 rounded-lg">
                                    ...
                                    <form>
                                        ...
                                        {selectedWithdrawCurrency && (
                                            <div className="text-sm text-primary dark:text-primary mb-4">
                                                ...
                                                {parseFloat(
                                                    tab === 'Locked'
                                                        ? selectedWithdrawCurrency.lockedBalance ||
                                                              '0'
                                                        : selectedWithdrawCurrency.availableBalance ||
                                                              '0',
                                                ).toFixed(6)}{' '}
                                                {
                         ...
                         */}
                                        ...
                                        {!shouldShowBundleForWithdraw &&
                                            userDetails?.withdrawFees && ( // Changed from withdrawFee to withdrawFees
